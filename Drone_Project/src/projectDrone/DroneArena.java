...
public class DroneArena {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i < dronesWanted; i++) // Has value representing amount of drones created.
        {
            ...
            if (i == xSize * ySize) {
                System.out.println("Arena at maximum capacity with: " + xSize * ySize
                        + " drones.\nCannot fulfil request for remaining " + (dronesWanted - xSize * ySize)
                        + " drones.\n");
                break;
            } else {
    ...
    public void addDrone() // Receives drone
    {
        ...
        randDir = Direction.getRandomDirection(); // Randomly generating a direction using method in enumeration class.
        ...
        if (droneAL.size() != xSize * ySize) // Just less than because droneAL starts from index 0. 2*2 grid can only
    ...
    public static boolean canMoveHere(int x, int y) {
        ...
        if (x == 0 || y == 0 || x == xSize + 1 || y == ySize + 1) // if new coords are correspond to any of these then
        {
            ...
            //System.out.println("Drone obstruction found: " + getDroneAt(x, y) + "Checking other directions.\n");
    ...
    public static Drone getDroneAt(int x, int y) {
        ...
        /**
         * Search droneAL for drone at x and y
         * Iterate through all drones and compare to new drone at x and y
         * if there is a drone then return drone at x,y
         * if not, return null;
         */
        ...
        for (int i = 0; i < droneAL.size(); i++) // iterate through all drones in al
        {
            ...
            if (droneAL.get(i).xPos == x && droneAL.get(i).yPos == y) // if drone i in al has same properties as drone then
            {
                //p("return drone");
    ...
    public static void moveAllDrones() {
        //System.out.println("Drone Arraylist size: " + droneAL.size());
        /**
         * Redraw ConsoleCanvas on screen.F
         */
        for (int i = 0; i < droneAL.size(); i++) // Iterate through all drones in arena.
        {
            droneAL.get(i).tryToMove();
            //Drone.tryToMove(droneAL.get(i)); // tryToMove every drone.
        }
    ...
    public boolean isHere(int sx, int sy) {
        ...
        for (int i = 0; i < droneAL.size(); i++) {
            if (droneAL.get(i).xPos == sx && droneAL.get(i).yPos == sy)// If x and y are used by other drones
            {
    ...
    public String toString() {
        ...
        sb.append("Arena's size is: " + xSize + " | " + ySize + "\n" + xSize * ySize + " spaces for drones.\n");
